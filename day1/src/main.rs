...
fn part_one(input: &str) -> u32 {
    ...
    let total_distance = col1.iter().zip(col2.iter()).map(|(a, b)| a.abs_diff(*b)).sum();
    total_distance
}
...
fn part_two(input: &str) -> u32 {
    ...
    let counts = col2.iter().fold(std::collections::HashMap::new(), |mut acc, &num| {
        *acc.entry(num).or_insert(0) += 1;
        acc
    });

    col1.iter().map(|num| num * counts.get(num).unwrap_or(&0)).sum()
}
